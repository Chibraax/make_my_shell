...
# Choice if the prompt
while :
  echo "1: ┌─[$USERNAME💀$HOST]⚡⚡[/some/random/path/]⚡⚡[0000]
    └─[$]"
  echo "2: ┌─[$USERNAME🎃$HOST]🩸🩸[/some/random/path]🩸🩸[0000]
    └─[$]"
  read -r "[+] What do you prefer 1,2 ?" choice
  if [[ $choice -neq "1" || $choice -neq "2" ]]; then
    echo "Bad choice"
    echo "Pick 1 or 2"
  fi
